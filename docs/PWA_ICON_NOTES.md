...
**先跑采集脚本** —— 下面各层的事实它一次采齐,并且**已把 `/health` 的 token 脱敏,输出可直接贴出**:

```bash
curl -fsSL https://raw.githubusercontent.com/3kaiu/dsh-pwa/main/scripts/pwa-doctor.sh | bash
# 已克隆仓库时: bash scripts/pwa-doctor.sh [PORT]
```

它只读、不改任何状态,并直接给出三类结论之一(**旧版守护** / **新版但发非官方 manifest** /
**已透传**)以及对应修法。**要请人帮忙判断时,把这个输出贴过去就够** —— 缺层是最常见的误判来源:
只贴一句「程序坞里的图标不对」,会把「旧版包装器」误判成「图标缓存」(本项目真实踩过一整轮:
对着一台机器的缓存查,真因在另一台机器的已装二进制里)。

它采的也正是手工排查要看的每一层:

1. **已装守护是哪一版** —— 二进制里是否还有自造路由。**这层最容易漏,也是本项目最常出问题的一层**;
   判据是**字符串**而不是文件大小(见下节「仍然不成立的一个直觉」)。
2. **官方资产本身是否正常** —— 直连 dsh(绕过守护)取 `/manifest.webmanifest` 与
   `/favicon.svg`,确认 dsh 自己发出来的就是预期的那份。
3. **守护是否真的在透传** —— 经守护端口取同样两个路径,响应应与上一步**逐字节一致**。
   若不一致,说明有拦截逻辑残留(这正是本项目撤掉自造 manifest 时要根除的状态)。
4. **app 包里的图标是什么** —— 用上面的 `sips` 导出,与官方 favicon 对照。
5. 以上都对但程序坞仍显示旧图标,则是「图标在添加那一刻定格」那条机制在起作用,按下一节重加。

**仍然不成立的一个直觉:用文件大小判断守护是哪一版。** 同一份源码编出来的二进制,加不加
ad-hoc 代码签名会差约 50KB(实测 120136 → 170624),而不同版本的签名产物大小又几乎相同。
判据只能是内容 —— `strings <二进制> | grep -c '/icon.svg'`,或直接用上面的脚本。
