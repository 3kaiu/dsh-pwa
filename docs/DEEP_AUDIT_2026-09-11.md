...
| 第 1 批第 3 项（**0 个 release**） | 需打 tag | ✅ **已解决（v0.3.3，2026-09-12）** | 见下方「S1 之后的真正阻断」 |
...
> **状态：✅ 已解决（2026-09-12）。** 在 `c6854d6` 打附注 tag **`v0.3.3`** 并推送，
> Release workflow `34647329968` 全绿（含新增的资产校验步骤），
> release 现已存在、两个资产可下载。下方为发现时的原始记录。
...
> **已落地（2026-09-12，`b0d6aff`）**：`release.yml` 在「发布」之后新增
> 「校验发布资产真的可下载」步骤 —— 带 5 次重试下载
> `releases/download/$TAG/dsh-pwa.zip{,.sha256}`，并**三方比对**
> 「清单哈希 = 下载物哈希 = 本地构建物哈希」，不一致即让 Release 作业变红。
> 放在 release 作业内（而非定时任务）的好处：只验「自己刚发的东西」，不会天天红。
> 仍缺的一环：**资产事后被删除**这类情况只有定时巡检才能发现（暂未加，避免常态噪声）。
...
> **状态（2026-09-12 复核）**：第 1、2、3 项**均已完成**
> （`e55263d` / `884891f` / `v0.3.3`）。发布后实测 `releases/latest/download/dsh-pwa.zip`
> 返回 **200（149133B）**，哈希与清单一致，README 两种安装方式恢复可用。
> 详见 §一之二。
...
| 3 | 打通首个 release（当前 0 个，`curl\|bash` 必然 404） | 打 tag 走 `release.yml` | 小 | ✅ `v0.3.3` |
