...
| E6 | 其余低危项(含连接数上限 E6d) | 记录时未修(2026-09-12 复核仍成立) |
...
| C2 | `cleanup-deps.sh` 删除面远大于探针覆盖 | 记录时未修(2026-09-12 复核仍成立:仅 `bash -n` 把关) |
...
| 深审 E6 | 其余低危项(含连接数上限 E6d) | P3,未修(2026-09-12 复核仍成立) |
| 深审 C2 | `cleanup-deps.sh` 删除面大于探针覆盖 | P2,未修(2026-09-12 复核仍成立) |
...
> 再决定动作,结论见上表「处置」列。仍未闭合的两条(E6、C2)已复核确认**当前依然成立**。
>
> 同一轮复核还发现一个未被任何条目记录的更大问题:`scripts/` 与 `tests/` 里 13 处
> `daemon.c:NNN` **行号引用系统性漂移** —— 只有 4 处仍指向所称内容,其余全部指向无关代码
> (例:某处称「setsid 自成进程组」,该行实际是 `} else {`)。已全部改为符号锚点(函数名),
> 并新增门禁禁止行号引用回归,见 `5503968`。
