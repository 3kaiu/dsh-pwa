...
| E5 | `pick_port_fd` 的「端口预留」实际不存在(注释与实现不符) | ✅ `5503968`(按实现改写函数头注释) |
| E6 | 其余低危项(E6a–E6f) | ✅ 已逐条处置 —— E6a/E6b/E6d 修复,E6c/E6e/E6f 记为已知限制(见「七、」下的 E6 子项表) |
...
> **已于 `5503968` 按实现改写函数头**(保留原意,明确写出「只缩小窗口,非互斥」)。
...
| C2 | `cleanup-deps.sh` 删除面远大于探针覆盖 | ✅ `d3adfa1`(新增删除面门禁 + `--dry-run`) |
| C3 | `install.sh:250` 布尔优先级无括号 | ✅ `5503968`(补显式括号,把左结合语义写死) |
...
| ID | 问题 | 处置 |
|---|---|---|
| D2 | 依赖 `python3`,而 macOS 12.3+ 不再自带;缺失时**静默降级** | ✅ `5503968`(缺 python3 时显式告警) |
| D3 | 硬编码回退 `LTS_VER="24.19.0"`,会随时间陈旧且无告警 | ✅ `5503968`(回退时显式告警,不再静默) |
| D4 | updater plist 缺 `NODE_OPTIONS=--use-system-ca` → 企业 TLS 代理下更新器可能**静默永不更新** | ✅ `b9dde78`(update-dsh.sh 加能力探测后按需追加) |
| D5 | `workflow_dispatch` 触发时 `github.ref_name` 是**分支名**,会建出以分支名命名的 release | ✅ `5503968`(显式 `inputs.tag` + 形状校验) |
...
| 来源 | 条目 | 备注 |
|---|---|---|
| 深审 A3 | 透传路径 CSRF 可被流水线请求绕过 | ✅ `5503968`(复核后按 P3 的另一选项写入 README 威胁模型) |
| 深审 E5 | `pick_port_fd` 函数头注释与实现不符 | ✅ `5503968` |
| 深审 E6 | 其余低危项(E6a–E6f) | ✅ **已逐条处置** —— E6a/E6b/E6d 修复,E6c/E6e/E6f 记为已知限制(见下) |
| 深审 C2 | `cleanup-deps.sh` 删除面大于探针覆盖 | ✅ `d3adfa1`(删除面门禁 + `--dry-run`) |
| 深审 C3 | `install.sh` 布尔优先级无括号 | ✅ `5503968` |
| 深审 D2 | 依赖 `python3`,缺失时静默降级 | ✅ `5503968` |
| 深审 D3 | 硬编码回退 LTS 版本,会陈旧且无告警 | ✅ `5503968` |
| 深审 D5 | `workflow_dispatch` 会用分支名建 release | ✅ `5503968` |
...
> **2026-09-12 逐条复核说明:** 上表原为「记录时未修」的快照,不是现状。按本项目既有教训
> (审计报告是快照 —— 曾把早已修好的 S1(P0) 照抄成待办继续传播),本次对每条都回到代码核对
> 再决定动作,结论见上表「处置」列。C2 已由 `d3adfa1` 闭合;E6 已按下面的子项表逐条处置。

### E6 逐条处置(E6a–E6f)

E6 原文只写「其余低危项」,本身不是可执行条目 —— 明细在合并前的深审文档里。下表是**回到
当前代码逐条复核**后的结论(原文档行号已全部漂移,故一律用符号锚点定位)。

| 子项 | 问题 | 处置 |
|---|---|---|
| E6a | 响应缺安全头;引导页内联 JS 无 CSP | ✅ 已修 —— `respond()` 是全站唯一响应出口,`nosniff` + 最小 CSP 集中加一次即覆盖全部响应 |
| E6b | `Content-Type` 来自参数,未来传用户数据即成 CRLF 注入面 | ✅ 已修 —— `respond()` 对含 CR/LF 的 `ct` 做净化(而非断言:生产里断言等于崩溃) |
| E6c | `extract_str` 用 `strstr("\"key\"")` 解析 JSON,值内含同名子串可误匹配 | 📝 已知限制 —— `run.json` 由本仓库写入,非不可信输入 |
| E6d | 每连接 `fork`,**无并发上限**(仅 `EMFILE` 时退避) | ✅ 已修 —— `MAX_CONN`(默认 256,`DSH_RT_MAX_CONN` 可覆盖),超限在 **fork 之前**直接 503 |
| E6e | `stop_dsh` 对非本进程 spawn 的 pid 仅校验「可执行文件是 node」 | 📝 已知限制 —— 需先能写 `dsh.pid`,同用户前提;交叉校验成本高 |
| E6f | 僵尸回收仅在 `poll` 返回后执行,最长滞留 `poll_ms` | 📝 已知限制 —— 影响有限,可接受 |

> **E6d 的两个实现要点**(都属「修一个洞别开另一个」):
> (a) 上限判定必须在 **`fork()` 之前** —— 放到子进程里判,限额就退化成「限制子进程存活数」,
> 而无界 fork 本身才是要堵的洞;
> (b) 父进程手里的 accepted socket 是**阻塞**的(超时只在子进程 `handle_conn` 里设),故 503
> 路径必须先设 `SO_SNDTIMEO` 再写 —— 否则一个「只连不读」的客户端能把主循环永久挂住,
> 于是「加限额」反而引入了与 A2 同类的挂死面。
>
> **E6a 的 CSP 与引导页是强耦合的**:CSP 一旦漏掉引导页需要的来源,页面就是**白屏**,而白屏
> 没有任何信号(与 E4 静默截断同类)。故新增门禁 `CSP covers every resource the boot page
> actually needs`:它从**引导页模板本身推导**所需指令与来源(而不是把指令名抄第二遍),
> 再核对真实响应头 —— 于是「模板加了新资源类型却忘了改 CSP」会在 CI 变红,而不是在用户浏览器
> 里变白屏。该门禁带负控(见 `daemon-cases.bats` 的 self-check 用例)。
