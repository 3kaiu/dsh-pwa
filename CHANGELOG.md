...
- **PWA 身份改为完全透传 dsh 官方 manifest,守护不再自造** — 原先守护会拦下 `/manifest.webmanifest` 与图标路径、自己发一份,理由是「dsh 的 manifest 会把 PWA 绑到它自己的内部端口」。该理由**不成立**:官方 manifest 的 `id` / `start_url` / `scope` 全是 origin 相对路径(`"/"`),经透传自然绑定到守护监听的端口,内部端口从不出现在 manifest 里。撤掉自造后一并删除 `/icon.svg`、`/favicon.svg`、`/icon.png` 三条拦截、内嵌的 PNG 位图与其 base64 解码器、以及 `respond_raw` / `respond_bytes` 拆分(二进制出口是它们唯一的存在理由)。为什么必须撤而不是留着兜底,见「Fixed」一条
- **README 的 PWA 体验描述改为可验证的表述** — 原文称「支持通知与离线缓存」,但 dsh 前端**没有注册 Service Worker**(已核对其前端产物),离线缓存不成立,Web Push 同样依赖 Service Worker。改为「名称、图标、显示模式全部沿用 dsh 官方 manifest」
- **README 精简** — 185 → 134 行(4,832 → 4,204 字符),内容不删只并:「推荐/快速安装」两节合为单一代码块、Requirements 的三条「依据」压成一段、Troubleshooting / Uninstallation / Development 的命令说明改行内注释、`Contributing` 并入 `Development`、删去仅复述首段链接的 Acknowledgements。**顺带补一处漂移**:README 与 `docs/TOOLS_INTEGRATION.md` 给出的提交前编译命令漏了 `-Wunused-macros` —— 该开关由审计 F11 引入并写进本地 gauntlet,而 CI 用的是不带它的 `-Wall -Wextra -Werror`,故这两处(描述**提交前**门禁)应与本地 gauntlet 对齐而非与 CI 对齐。CHANGELOG 对 README「开发」段与卸载一节的引用均已保留
