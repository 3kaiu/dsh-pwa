...
# 阈值 150000 是「双架构 + 内嵌引导页」的合理上界(实测约 121KB,与未内嵌任何资产的基线
# 持平);真的合理增长时应当**显式上调阈值**并说明原因,而不是靠 info 静默放过。
# 前车之鉴:曾把一份 4KB 图标位图 base64 内嵌进来,二进制随即从 120200 涨到 136664 ——
# 内嵌字符串的二进制开销约为其 base64 长度的 **2.8 倍**,估体积时别按源码字符数算。
# 该图标已随「PWA 身份改为一律透传 dsh 官方资产」一并移除(见 docs/PWA_ICON_NOTES.md),
# 故此处的余量是真实余量,不是被资产吃掉后剩下的零头。
if [ -f "$TMPD_BUILD/daemon" ]; then
